...
M = readmatrix([path,'268deg28_3.csv']);
...
Q(Q<9)=0;
counts = repelem(PhaseDiff5,Q);

f=figure;
histogram(counts);

phase_CH@(t>(max(t)-50e-6))=[];
phase_CH@(t<(50e-6))=[];

counts2 = weighted_counts(phase_CH2,amp2_2,10,2);
counts8 = weighted_counts(phase_CH8,amp2_8,10,2);
...
function counts = weighted_counts(PhaseDiff5,amp_env,bins,cutoff)
amp_max = max(amp_env);
Q = floor(amp_env.*bins./amp_max);
Q(Q<cutoff)=0;
counts = repelem(PhaseDiff5,Q);
end
